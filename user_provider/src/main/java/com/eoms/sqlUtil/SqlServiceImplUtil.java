...
public class SqlServiceImplUtil extends SqlUtil {
    ...
    public SqlServiceImplUtil(){
    }
    /**
     * 构造函数
     */
    public SqlServiceImplUtil(String tablename,String packageOutPath){
        try {
            ...
            try {
                ...
                String outputPath = directory.getAbsolutePath() + getDefaultPath()
                        + (packageOutPath+".service.impl").replace(".", "/") + "/"+ initcap(tablename) + "ServiceImpl.java";
                ...
                pw.flush();
                pw.close();
                System.out.println(initcap(tablename)+"ServiceImpl 执行完毕，生成路径为："+outputPath);
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    private String parse(String[] colnames, String[] colTypes, int[] colSizes,String tablename,String packageOutPath) {
        ...
        // 生成package包路径
        sb.append("package " + packageOutPath + ".service.impl;\r\n");
        sb.append("\r\n");
        ...
        // 注释部分
        sb.append("   /**\r\n");
        sb.append("    * @文件名称：" + this.initcap(tablename) + "ServiceImpl.java\r\n");
        sb.append("    * @创建时间：" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) + "\r\n");
        sb.append("    * @创  建  人：" + getAuthorName() + " \r\n");
        sb.append("    * @文件描述：" + this.initcap(tablename) + "ServiceImpl 接口类\r\n");
