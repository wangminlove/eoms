...
public class SqlDaoUtil extends SqlUtil {
    ...
    public SqlDaoUtil(String tablename, String packageOutPath){
        try {
            ...
            try {
                ...
                String outputPath = directory.getAbsolutePath() + getDefaultPath()
                        + (packageOutPath+".dao").replace(".", "/") + "/" + initcap(tablename) + "Dao.java";
                FileWriter fw = new FileWriter(outputPath);
                PrintWriter pw = new PrintWriter(fw);
                pw.println(content);
                pw.flush();
                pw.close();
                System.out.println(initcap(tablename)+"Dao 执行完毕，生成路径为："+outputPath);
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    private String parse(String[] colnames, String[] colTypes, int[] colSizes,String tablename,String packageOutPath) {
        ...
        // 生成package包路径
        sb.append("package " + packageOutPath + ".dao;\r\n");
        sb.append("\r\n");
...
//    }
    ...
    public static void main(String[] args) {
        new SqlDaoUtil("sys_users","com.eoms");
